"""Google AI Studio (api-key) backed Gemini client. Drop-in replacement for
vertex_gemini for users who don't have a GCP project and just want the cloud
Gemini family — surfaced by the onboarding wizard.
"""
...
def _client():
    ...
    if _genai_client is None:
        key = cloud_credentials.load_gemini_api_key()
        if not key:
            raise RuntimeError(
                "No Gemini API key configured. Run the cloud onboarding wizard."
            )
