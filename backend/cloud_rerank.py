"""Cross-clip ranking via Gemini through the AI Studio API key (cloud backend). Drop-in replacement for vertex_rerank that doesn't need GCP project access — pairs with cloud_gemini.py."""
...
def rerank_job(clip_reviews: List[Any]) -> int:
    ...
    for segment, group in by_seg.items():
        ...
        rank_order = cloud_gemini.rerank_segment(segment, payload)
        if rank_order:
        ...
        if use_filename_order:
            ...
            items = cloud_gemini.order_segment(segment, payload)
            if items:
